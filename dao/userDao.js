...
module.exports = {
    add: function (req, res, next) {
        pool.getConnection(function(err, connection) {
            ...
            connection.query($sql.insert, [id, param.username, password], function(err, result) {
                if(result) {
    ...
    queryById: function (req, res, next, nextMyself) {
        ...
        pool.getConnection(function(err, connection) {
            connection.query($sql.queryById, username, function(err, result) {
                ...
                connection.release();
                nextMyself(result.length === 0, self);
            });
        });
    },
    login_correct: function (req,res,next) {
        var id = +req.query.id; // 为了拼凑正确的sql语句，这里要转下整数
        var password = req.query.password;
        //var param = req.query || req.params;
       /* console.log(req.query.password);
        var content = 'password';  //加密明文
        var md5 = crypto.createHash('md5');//定义加密方式，md5不可逆
        md5.update(content);
        var password = md5.digest('hex'); //加密后的upassword
        console.log("============================================="+password);*/
        console.log(req.query.password);
        var p = {id:id,password:password};
        console.log(p);
        let self = this;
        console.log("=============queryBYId111===========");
        pool.getConnection(function(err, connection) {
            connection.query($sql.query_correct, p,function(err, result) {
                console.log("=============queryBYId2222===========");
                var a = result;
                console.log(password);
                console.log(err);
